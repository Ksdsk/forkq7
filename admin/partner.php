...
if(isset($_GET['id'])){
  if(mysqli_query($connect, 'DELETE FROM partner WHERE partner_id ='.$_GET['id'])) {
...
if(isset($_POST['submit'])) {
  ...
  if(mysqli_query($connect, 'INSERT INTO partner(`partner_name`) VALUES ("'.$copy['partnerName'].'")')) {
    mysqli_close($connect);
    header('Location: '.$_SERVER['PHP_SELF']);
  } else {
...
?>
      ...
      <div class="col-sm-9 mt-5">
        <p class="bg-dark text-white p-2">List of Partners</p>
        <table class="table table-striped">
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_array($result)) {
              ...
              <td><a href="'.$_SERVER['PHP_SELF'].'?id='.$row['partner_id'].'">Delete</a></td>
              </tr>';
              $index++;
            }
            ?>
    ...
    <div>
      <button type="button" class="btn btn-danger icons fs-4 py-1" data-bs-toggle="modal" data-bs-target="#addPartner">+</button>
    </div>
  </div>
